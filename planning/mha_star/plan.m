function [output, vm, p] = plan(alpha, map_, start, goal, plotPath)

alpha %scale between heuristics
global open_a;
global open_h;
global bp;
global closed_a;
...
%load('/Users/sanghamitra/Desktop/Academic/WettergreenResearch/AdaptiveExploration/planning/featureSpace_module/testData/testData_8.mat');
...
start_id= sub2ind(size(valuemap), start(1,1), start(1,2));
goal_id= sub2ind(size(valuemap), goal(1,1), goal(1,2));
hmap=zeros(size(valuemap, 1), size(valuemap, 2));
mapSize=size(hmap)
...
hmap = getHmap(start_config',valuemap); %diff
%hmap = getHmap(start_config',valuemap, 10); %fspace
