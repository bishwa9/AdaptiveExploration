...
% finding variance across all channels for the whole map with the starting
% point
start_point=[1 1];
vect_channel = zeros(size(valuemap,1)*size(valuemap,2),3);
start_vals=zeros(1,3);
for i=1:nvisiblechans
    vect=valuemap(:,:,i);
    vect_channel(:,i)=vect(:);
    start_vals(1,i)=valuemap(start_point(1,1),start_point(1,2),i);
end
start_vals=repmat(start_vals,size(vect_channel,1),1);
V = zeros(size(valuemap,1)*size(valuemap,2),3);
for i=1:nvisiblechans
    V(:,i)=var([start_vals(:,i) vect_channel(:,i)],0,2);
end
