...
sigma = 0.25*(max(dists(pointToSample_ind,:)) - d);
...
if redoClustering == 1
    if do_meanShift == 1
        ...
        [memberships,centers,~,dists] = kmeans(feature_vector_new,nclasses);
    end
else
...
[prob, entropy_dinv] = calc_entropy(centers, feature_vector); 
...
for idx = 1:size(sampled_points,1)
    ...
    entropy_dinv(sampled_points(idx, 1), sampled_points(idx, 2)) = -Inf; % makes sure the same point isn't sampled everytime!
    
    if idx ~= size(sampled_points,1)
        idx_ = sub2ind(size(entropy_dinv), sampled_points(idx,1), sampled_points(idx,2));
        feature_vector_new(idx_,:) = feature_vector(idx_,:); %make sure the sampled point doesn't move in feature space.
    end
end
